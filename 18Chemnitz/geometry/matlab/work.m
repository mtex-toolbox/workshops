...
oS = sigmaSections(ori.CS,ori.SS);

odf = calcODF(ebsd('Fo').orientations,'halfwidth',10*degree);
plotSection(odf,oS,'contourf')

%grains = calcGrains(ebsd)
lg = grains(grains.area > 50000)
lg = lg('Fo')

h = Miller({0,0,1},{1,0,0},cS.CS);

cS = 0.5 * sqrt(lg.area/max(lg.area)) .* (lg.meanOrientation .*  cS); 

plot(lg.meanOrientation,cS,'add2all') 
plot(lg.meanOrientation,cS(h(1).symmetrise),'add2all','faceColor','red') 
plot(lg.meanOrientation,cS(h(2).symmetrise),'add2all','faceColor','green') 

saveFigure('pic/odfOriSmoothSigmacS.png','crop')
...
plotSection(odf,'axisAngle',(10:15:115)*degree,'colorRange',[0,22],'upper')
...
%saveFigure('pic/quartz.png')

%%

hold on
mori = orientation('axis',Miller(1,0,-1,1,cs),'angle',87*degree)
plot(mori * cS,'facecolor','green')
hold off

saveFigure('pic/twins.png')

%%

mtexdata dubna
odf = calcODF(pf)
ori = odf.discreteSample(20)
%%


h = Miller({1,0,0,0},{0,0,0,1},cs)
plotPDF(odf,h,'contour')

plot(ori,ori*cS*0.3,'add2all')
plot(ori,ori*cS(h(1))*0.3,'faceColor','red','add2all')

pos = [596   296   322   644]
set(gcf,'Position',pos);
%saveFigure('pic/pfcS.png')

saveFigure('pic/pfcSred.png')
